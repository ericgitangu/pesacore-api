...
namespace PesaCore.Controllers;
...
public class CqrsController : ControllerBase
{
    ...
    // See: Behaviors/IdempotencyBehavior.cs and walkthrough.md Step 5b.
    //
    ...
    // COMMAND — opens a new account. Same pipeline as transfer: it's a mutation, so the
    // IdempotencyBehavior REQUIRES X-Idempotency-Key (a replayed POST returns the cached
    // CreateAccountResult instead of opening a duplicate). FluentValidation
    // (CreateAccountValidator) runs first; the handler only sees valid input.
    //
    // Returns 201 Created with a Location header pointing at the new account's balance
    // (the canonical GET for the resource) — the correct REST shape for a create.
    [HttpPost("accounts")]
    public async Task<IActionResult> CreateAccount([FromBody] CreateAccountCommand command)
    {
        ...
        // Guard: CreatedAtAction builds a Location URL from the route value, and a null
        // AccountNumber would throw while generating the link → a 500 on an otherwise
        // successful create. If the handler reports success but somehow has no number,
        // fall back to a plain 200 with the body rather than crashing the response.
        // (Largely mitigated upstream by per-type idempotency scoping, but guarded here.)
        if (string.IsNullOrEmpty(result.AccountNumber))
            return Ok(result);

        return CreatedAtAction(
            nameof(GetBalance),
            new { accountNumber = result.AccountNumber },
            result);
    }
}
